  ...
  return (
	<div className="bg-black/20 border border-white/50 rounded-[24px] pl-[10px] pr-[20px] py-[30px] flex gap-[15px] lg:gap-[30px]">
		...
		<div className="flex-1">
			<h1 className="tex-[24px] font-medium">{item.name}</h1>
			<Selector selectedType={selectedType} handleType={handleType}/>

			<div className="mt-5 flex justify-between">
				<p>zł{item.price} / scoop</p>

				<button onClick={handleBasket}
				className={`border py-1 px-3 rounded-md hover:bg-white/30 transition ${!selectedType && "invisible"}`}>Add to Basket</button>
			</div>
		</div>
	</div>
  )
